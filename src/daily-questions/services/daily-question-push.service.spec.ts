...
describe('DailyQuestionPushService', () => {
  ...
  it('uses one stable daily dedupe key per tenant, schedule and user', async () => {
    const payload = {
      title: 'Napi kérdőív',
    ...
    expect(notifications.sendNow).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        dedupeKey: 'tenant-1:daily_question:2026-06-15:schedule-1:user-1',
      }),
    );
    expect(notifications.sendNow).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        dedupeKey: 'tenant-1:daily_question:2026-06-15:schedule-1:user-2',
      }),
    );
    expect(notifications.sendNow).toHaveBeenNthCalledWith(
      3,
